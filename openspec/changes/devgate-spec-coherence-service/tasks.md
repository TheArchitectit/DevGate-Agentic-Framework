...
- [x] Stage-2 signing inside the containerized path: the signer secret never reaches the container (coh-ev-01, coh-rt-04). **CLOSED 2026-09-26, same day it was found.** The gap (below) was real and measured; the fix chose the forward-through-driver option — the smallest change consistent with the sealing order, which puts the attestation inside the container's sealed envelope. `container_exec.run_containerized` now reads `HUB_COHERENCE_SIGNER_KEY` / `_KEY_ID` / `_IDENTITY` host-side and forwards them through the launcher's existing `env` kwarg (launcher.py:262-276), only when the host actually holds each var — omission stays the fail-closed default, and `attest.py`'s contract (no key → exit 33) is untouched. The static-deny allowlist (`TestStaticDefaultDeny.test_environ_read_only_by_the_signing_module`) gained `container_exec.py` as a deliberate control-plane secret holder — the test's own comment prescribes exactly this path for that exact reason; the module never touches the evaluation path. The secret-in-container-env exposure was weighed against the escape review: the signer key sits in a container env var inside the same isolation boundary the evaluator already controls (no-new-privileges, cap-drop ALL, network=none, read-only rootfs) — the same trust position `HUB_COHERENCE_EVALUATOR_IMAGE_DIGEST` has always had; a host-side signing step would have moved the decision bytes OUT of the sealing order, a bigger contract change. TDD: `test_signer_env_reaches_the_container` RED first (asserted all three vars captured by the launcher's env), `test_signer_env_absent_forward_nothing` pins the omission-is-default; the drop-the-forwarding mutant is killed by the first test. Full suite 1053 green. **E2E on ucs03** (the real proof, not a mock): stage-2 fresh-promotion fixture through the template's two commands with the signer env set — driver exit 0, `attestation.json` sealed with signer `pilot-signer/signer-1`, and `--verify-run` against the matching signer-set document returned **`verification ok`**. Criterion 7's signed-attestation evidence now covers the containerized path too. Residual, recorded honestly: the CI template still does not supply the signer vars (control-plane secret distribution is Phase-3 hub-fetch scope, and the template correctly fails closed without them); the outputs-dir uid trap below still stands.
  **The gap as found (2026-09-26 morning):** `attest.seal_run` executes INSIDE the container (design.md:98 sealing order), so `HUB_COHERENCE_SIGNER_KEY` / `_KEY_ID` / `_IDENTITY` had to be present in the container's environment for any Stage-2 fresh-promotion run to sign. The driver (`container_exec.run_containerized`) forwarded only `HUB_COHERENCE_EVALUATOR_IMAGE_DIGEST` and the launch config carries no env field, so every containerized Stage-2 run failed closed exit 33 `signer-key-not-configured` — measured on ucs03 (driver exit 33, error class `attestation`). The failure was correct fail-closed behavior; the defect was that no code path could ever satisfy it.
- [x] Container outputs-dir writability under rootless podman: the launch `user: 1000:1000` is not the host's uid 1000 (coh-rt-01 operator surface). **CLOSED 2026-09-26, same day it was found.** Under rootless podman with a subuid map (container uid 1000 → host uid 100000), an outputs directory created by the host user (uid 1000, mode 755) is NOT writable by the container user; the in-container CLI falls back to `emit_with_fallback` and writes result.json into the container's own `/tmp` — a location that vanishes with `--rm`. Both hosts (workstation, ucs03) needed an explicit `chmod 777` before the container could write. Fixed at both invocation surfaces the operator actually uses: the CI template's execution step (`chmod 777 "$OUT"` right after `mkdir -p`, with a comment naming the subuid cause) and `scripts/coherence-local` (`out.chmod(0o777)` after mkdir) — the outputs dir only; every input mount stays read-only. E2E proof on the workstation: `coherence-local` with a fresh, never-hand-touched outputs dir now produces the full PASS bundle where the identical invocation previously required a manual chmod. Deliberately NOT changed: the launch config still pins `user: 1000:1000` (the isolation posture is correct; the bug was host-side dir ownership, not the container user) and `emit_with_fallback` keeps its message (the fallback path exists for genuinely unwritable output targets; the operator trap is closed upstream by making the trap not fire in the supported invocation paths). Triage note for the residual corner (a caller who sets up their own outputs dir): "result written to fallback location" + no result.json on the host = outputs-dir uid mismatch, not a launcher bug.
